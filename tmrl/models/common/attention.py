...
class Attention(nn.Module):
    ...
    def forward(self, x, pos_embed=None, attn_mask=None):
        ...
        if self.use_sdpa:
            x = F.scaled_dot_product_attention(q, k, v, attn_mask, dropout_p=self.attn_drop.p, is_causal=self.is_causal)
        else:
            ...
            if self.is_causal:
                ...
                block_diag_mat = torch.block_diag(*[self.causal_block_mat for _ in range(num_blocks)])
                triu_mat = torch.triu(torch.ones(N, N, device=x.device), diagonal=1).bool()
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, c, x_pos_embed=None, c_pos_embed=None):
        ...
        B, Nc, D = c.shape
        kv = self.kv(c).reshape(B, Nc, 2, self.num_heads, D // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv.unbind(0)
